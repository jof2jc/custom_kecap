...
def _execute(filters, additional_table_columns=None, additional_query_columns=None):
	...
	for inv in invoice_list:
		...
		row = [
			inv.name, inv.posting_date, inv.customer, inv.sales_person,inv.territory, inv.item_code, inv.item_name,
			inv.qty, inv.uom
		]
...
def get_columns(invoice_list, additional_table_columns):
	...
	columns = [
		_("Delivery Note") + ":Link/Delivery Note:120", _("Posting Date") + ":Date:80",
		_("Customer") + ":Link/Customer:120", _("Sales Person") + ":Link/Sales Person:100",
		_("Territory") + ":Link/Territory:100",		
		_("SKU") + ":Link/Item:100", _("Item Name") + "::120", _("Qty") + ":Float:50", _("UOM") + ":Link/UOM:50"
		]
...
def get_invoices(filters, additional_query_columns):
	...
	return frappe.db.sql("""select si.name, si.posting_date, si.customer, si.sales_person, si.territory, it.item_code, it.item_name, it.qty, it.uom
				from `tabDelivery Note` si, `tabDelivery Note Item` it
